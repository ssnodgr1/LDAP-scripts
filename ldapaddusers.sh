...
ADMINPW="`dirname "$0"`/ldap-admin-password.txt"
# Find the largest used LDAP UID
UIDNUMBER="`ldapsearch -D cn=admin,dc=hades,dc=lab -y ${ADMINPW} -x -b 'dc=hades,dc=lab' uidNumber | grep uidNumber | cut -f 2 -d ' '|sort -n|tail -n 1`"
...
# Add LDAP entry
cat << EOF|ldapadd -v -x -D cn=admin,dc=hades,dc=lab -y ${ADMINPW} >> /dev/null
...
mkdir /network-storage/"$USERNAME" > /dev/null
chown -vR "$UIDNUMBER":500 /network-storage/"$USERNAME"/ #change the ownership of the newly created directory
chmod -v 700 /network-storage/"$USERNAME"/ #set the ownership on the newly created directory.
done < /tmp/ldap.tmp
rm /tmp/ldap.tmp
